describe('Задание №3.1. Формирование строки корзины', () => {
    ...
    it('Тест 3.1.2. Формирование строки 1', () => {
        const expected = '<div class="cart-item"><div>Пицца</div><div>количество 5 × цена 500.00 ₽ = 2500 ₽</div></div>';
        const received = renderCartItem({title: 'Пицца', count: 5, price: '500.00'});
        expect(received).toBe(expected);
    });
    it('Тест 3.1.3. Формирование строки 2', () => {
        const expected = '<div class="cart-item"><div>Эчпочмак</div><div>количество 2 × цена 55.00 ₽ = 110 ₽</div></div>';
